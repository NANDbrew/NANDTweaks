    ...
    internal class PortOfficePatches
    {
        ...
        private static class PortDudePatch
        {
            ...
            public static void AddInteriorEffectsTrigger(PortDude __instance, Port ___port)
            {
                ...
                if (AssetTools.bundle2)
                {
                    var prefab = AssetTools.bundle2.LoadAsset<GameObject>("triggers " + portIndex + ".prefab");
                    if (prefab != null)
                    {
                        UnityEngine.Object.Instantiate(prefab, ___port.transform, false);
                        Debug.Log("NANDtweaks: loaded triggers for port " + portIndex);
                        return;
                    }
                    else { Debug.LogWarning("nandtweaks: no triggers in asset bundle for port " + portIndex); }
                }
